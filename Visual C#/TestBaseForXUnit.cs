...
namespace UnitTestsProject
{
  public abstract class TestBase
  {
    ...
    private MethodInfo GetMethod<T>(T classRef, string methodName, BindingFlags flags)
    {
      ...
      {
        AssertFail("Method name cannot be null or whitespace");
      }
      ...
      if (method == null)
      {
        AssertFail($"{methodName} method not found.");
      }
    ...
    /// <summary>
    /// XUnit explicitly does not have an Assert.Fail method, this is a work around for that.
    /// </summary>
    /// <param name="message">Message to display on forced failure.</param>
    protected void AssertFail(string message) => Assert.True(false, message);

    /// <summary>
    /// XUnit explicitly does not have an Assert.Pass method, this is a work around for that.
    /// </summary>
    /// <param name="message">Message to display on forced pass.</param>
    protected void AssertPass(string message) => Assert.True(true, message);
  }
}
